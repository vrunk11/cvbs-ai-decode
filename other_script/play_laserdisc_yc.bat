...
if %efm% NEQ 0 %command% -A "%~dp1%name%.efm.pcm" -FstartG %time% -audioOffset %a_offset% -syncA R -pipeMode A | ffmpeg -f s16le -ac 2 -i -  -f wav - | ffplay - & pause & goto :eof
...
%command% -A "%~dp1%name%.pcm" -FstartG %time% -audioOffset %a_offset% -syncA G -pipeMode A | ffmpeg -f s16le -ac 2 -i -  -f wav - | ffplay -