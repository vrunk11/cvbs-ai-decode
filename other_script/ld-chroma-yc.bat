...
set chromafile=%~dp1%name%_chroma.tbc
if %ai-chroma% EQU 0 if exist %~dp1%name:_aiyc=%.tbc set chromafile=%~dp1%name:_aiyc=%.tbc

if "%name:~-5%" EQU "_aiyc" set name=%name:_aiyc=%

set lumafile=%~dp1%~n1.tbc

set "audiofile= "
if exist %~dp1%name%.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%name%.pcm
if exist %~dp1%name%_cx.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%name%_cx.pcm
if exist %~dp1%name%.efm.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%name%.efm.pcm

if exist %~dp1%~n1.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%~n1.pcm
if exist %~dp1%~n1_cx.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%~n1_cx.pcm
if exist %~dp1%~n1.efm.pcm set audiofile= -f s16le -r 44.1k -ac 2 -i %~dp1%~n1.efm.pcm
